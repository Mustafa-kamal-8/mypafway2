...
export default function ProductsPage() {
      ...
      complete: (results: {
        ...
        results.meta.fields.forEach((header) => {
          ...
          const matchedField = DB_FIELDS.find(
            (field) => lowerHeader === field.toLowerCase() || lowerHeader.includes(field.toLowerCase()),
          )
          if (matchedField) {
        ...
        setMappingStep(true)
        setError(null)
        console.log(`Loaded ${results.data.length} rows from CSV with headers: ${results.meta.fields.join(", ")}`)
      },
  ...
  const validateMapping = () => {
    const mappedDbFields = new Set(Object.values(columnMapping).filter(Boolean))
    const missingRequiredFields = REQUIRED_FIELDS.filter((field) => !mappedDbFields.has(field))
...
const handleUpload = async () => {
  ...
  try {
    ...
    while (i < totalProducts) {
      ...
      currentBatch++;
      setMessage(`Uploading batch ${currentBatch} (${batch.length} products)...`);
      console.log(`Uploading batch ${currentBatch} with ${batch.length} products`);
    ...
    setMessage(`❌ Upload failed. Error: ${err instanceof Error ? err.message : String(err)}`);
        ...
        <Dialog.Panel className="bg-white p-6 rounded-md shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {!mappingStep ? (
            ...
            <>
              <p className="text-sm text-gray-600 mb-4">
                Match your CSV columns with the database fields. This ensures your data is correctly imported.
              </p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left font-medium text-gray-500">Database Field</th>
                    </tr>
                  </thead>
                          ...
                          <div>
                            <select
                              ...
                              className={`w-full border rounded px-2 py-1 text-sm ${
                                fieldValidationErrors[header] ? "border-red-500" : ""
                              }`}
                            >
              ...
              {csvProducts.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-sm font-medium mb-2">Preview ({csvProducts.length} total rows)</h3>
                    ...
                    <table className="w-full text-xs">
                          ...
                          {csvHeaders.map((header) => (
                            <th key={header} className="px-2 py-1 text-left font-medium text-gray-500">
                              ...
                              {columnMapping[header] && (
                                <span className="block text-xs text-green-600">→ {columnMapping[header]}</span>
                              )}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody className="divide-y">
                        {csvProducts.slice(0, 3).map((row, idx) => (
                          <tr key={idx}>
                            {csvHeaders.map((header) => (
                              <td key={`${idx}-${header}`} className="px-2 py-1 truncate max-w-[150px]">
                                {row[header] || "-"}
                              </td>
                            ))}
                          </tr>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            {mappingStep ? (
              ...
              >
                {loading ? "Processing Batches..." : `Upload All (${csvProducts.length} products)`}
              </button>
            ) : (
              <button
